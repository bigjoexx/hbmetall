...
function getKopfplattePricesData() {
  const table = document.getElementById("kopfplatte-prices-table");
  const rows = table.rows;
  const kopfplattePricesData = {};

  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
    const option = row.cells[0].innerText;
    const value = row.cells[1].innerText;
    const price = parseFloat(row.cells[2].innerText);

    if (!kopfplattePricesData[option]) {
      kopfplattePricesData[option] = {};
    }
    kopfplattePricesData[option][value] = price;
  }

  return kopfplattePricesData;
}
...
async function main() {
  const kopfplattePricesData = getKopfplattePricesData();
