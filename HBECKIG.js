  ...
  function addToCart(formData, productData) {
    ...
    const foxyWeight = Math.round(finalWeight / quantity);
    const foxyData = {
...
function calculateFinalWeight(selectedValues, pricesData, traegerMenge, steelDensity) {
  ...
  const finalWeight = ((beamTotalWeight + kopfplatteWeight + fusplatteWeight) * traegerMenge).toFixed(2);
...
function calculateFinalSurface(selectedValues, traegerMenge, pricesData) {
  ...
  const finalSurface = ((beamSurface + kopfplatteSurface + fusplatteSurface) * traegerMenge).toFixed(2);
