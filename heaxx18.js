...
function getSelectedValues() {
  const maßen = document.getElementById("maßen").value;
...
function calculateLengthPrice(maßen, beamLength, pricesData) {
  if (!pricesData.hasOwnProperty(maßen)) {
    console.error("HEA size not found in prices data:", maßen);
    return 0;
  }

  const kgPerMeter = parseFloat(pricesData[maßen]["kg/m"]);
  const beamLengthInMeters = beamLength / 1000;
  const weight = kgPerMeter * beamLengthInMeters;
  const pricePerKg = parseFloat(pricesData[maßen]["kg"]);
...
function calculateBeamTotalWeight(selectedValues, pricesData) {
  const maßen = selectedValues.maßen;
...
function calculateOptionsPrice(selectedValues, pricesData) {
  ...
  const stegblechePrice = parseFloat(pricesData[maßen]["stegbleche"]) * heaStegbleche;
  const ausklinkungenPrice = parseFloat(pricesData[maßen]["ausklinkungen"]) * heaAusklinkungen;
  const bugelPrice = parseFloat(pricesData[maßen]["bugel"]) * heaBugel;
  ...
  const weiteresPrice = parseFloat(pricesData[maßen][weiteresOption]);
...
const bohrungenPricePerHole = parseFloat(pricesData[maßen][diameterCategory]);
...
function calculateTotalPrice(selectedValues, pricesData, beamMenge, kopfplattePrice, fusplattePrice) {
  ...
  const lengthPrice = calculateLengthPrice(selectedValues.maßen, parseInt(selectedValues.beamLength), pricesData);
  ...
  const baseCuttingPrice = parseFloat(pricesData[selectedValues.maßen]["schneiden"]);
...
async function main() {
  ...
  document.getElementById("maßen").addEventListener("change", () => updatePrice(pricesData));
