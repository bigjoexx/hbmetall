...
function createCircle(svg, svgns, x, y, diameter) {
  ...
  // Initially set the circle opacity to 0
  circle.style.opacity = '0';
  svg.appendChild(circle);

  // After the circle is appended to the SVG, make it visible
  setTimeout(() => {
...
const handleInputChange = async () => {
  ...
  // Await for the distance update before calculating the platteAbstand
  await updateMinDistance();
  // Now that the "Abstand" field's value is guaranteed to have been updated, proceed with creating the SVG and circles
  const abstand = parseInt(document.getElementById("platte-abstand").value, 10) || 0;

  let platteAbstand = Math.max(
