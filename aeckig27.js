...
// Determine the available options based on thickness
let availableOptions = [];
if (thickness < 6) {
  availableOptions = ['3 mm', '5 mm'];
} else {
  availableOptions = ['5 mm', '7 mm'];
}

// Set the selected value to the lower option
kehlnahtstarkeSelection.value = availableOptions[0];
...
// Change the order of the options in the select element
if (availableOptions[0] !== selectedValueKehlnahtstarke) {
  const optionToMove = kehlnahtstarkeSelection.querySelector(`option[value="${availableOptions[0]}"]`);
  kehlnahtstarkeSelection.insertBefore(optionToMove, kehlnahtstarkeSelection.firstChild);
}
