  ...
  function getProductData() {
    ...
    const productCode = "hbipe"; // Replace with the CMS code for the product
    const productName = "IPE Stahlträger"; // Replace with the CMS name for the product
    const imageUrl = "https://uploads-ssl.webflow.com/6401aa505499a5c8687df64f/645aa5a095ada613c54f29ae_IPE.png"; // Replace with the CMS image URL for the product
...
async function fetchPrices() {
  ...
  const response = await fetch("https://api.airtable.com/v0/appIIKEo5ExPVPr9I/IPE?api_key=keyLAGDgC4VT8YzLb");
...
function calculateTotalPrice(selectedValues, pricesData, traegerMenge, kopfplattePrice, fusplattePrice, finalWeight, finalSurface) {
  ...
  const totalPrice = Math.round((laengePrice + optionsPrice + baseCuttingPrice + kopfplattePrice + fusplattePrice) * traegerMenge + feuerverzinkungPrice + vorgrundierungRotPrice + vorgrundierungGrauPrice);
...
function updateDisplayedPrices(oldPriceWithoutVAT, oldPriceWithVAT, totalPrice, traegerMenge) {
  const priceWithoutVAT = Math.round(totalPrice);
  const priceWithVAT = priceWithoutVAT + (priceWithoutVAT * VAT_RATE);
  const foxyPrice = Math.round(priceWithoutVAT / traegerMenge);
