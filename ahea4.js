...
if (diameter >= 6 && diameter <= 18) {
  ...
  const bohrungenSelect = document.getElementById("bohrungen");

bohrungenSelect.addEventListener("change", function() {
    toggleBohrungenPositionContainer(this.value);
});

  function toggleBohrungenPositionContainer(selectedValue) {
    const bohrungenPositionContainer = document.getElementById("bohrungen-position-container");

    if (selectedValue === '2') {
        bohrungenPositionContainer.style.display = "block";  // Show the element
    } else {
        bohrungenPositionContainer.style.display = "none";   // Hide the element
    }
}
...
const bohrungenPrice = bohrungenPricePerHole * bohrungen;
  ...
  return optionsPrice;
  toggleBohrungenPositionContainer(selectedValue)
}
