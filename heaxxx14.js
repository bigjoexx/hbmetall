...
function calculateBeamSurface(selectedValues, pricesData) {
  ...
  console.log("Traeger format in calculateBeamSurface:", traegerFormat);
...
function calculateTotalPrice(selectedValues, pricesData, traegerMenge, kopfplattePrice, fusplattePrice, finalWeight) {
  if (selectedValues.laenge <= 0 || selectedValues.traegerMenge <= 0) {
    return { totalPrice: 0, feuerverzinkungPrice: 0 };
  }
  ...
  const totalPrice = (laengePrice + optionsPrice + baseCuttingPrice + kopfplattePrice + fusplattePrice + feuerverzinkungPrice) * traegerMenge;
    console.log("Total price:", totalPrice);
  
  return { totalPrice, feuerverzinkungPrice };
}
...
function calculateFinalSurface(selectedValues, traegerMenge) {
  if (selectedValues.laenge <= 0 || selectedValues.traegerMenge <= 0) {
    return 0;
  }
...
async function updatePrice(pricesData, checkboxChanged = false) {
  ...
  calculateFinalSurface(selectedValues, selectedValues.traegerMenge);
  ...
  console.log("Selected values in updatePrice:", selectedValues);
