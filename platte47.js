...
function createSVG() {
  ...
  const bohrungen = parseInt(document.getElementById("kopfplatte-bohrungen").value, 10);
  const bohrungenDurchmesser = parseInt(document.getElementById("kopfplatte-bohrungen-durchmesser").value, 10);
  const platteAbstand = parseInt(document.getElementById("platte-abstand").value, 10);
    
  // SVG Namespace
  const svgns = "http://www.w3.org/2000/svg";
  let svg = document.createElementNS(svgns, "svg");

  // Set viewBox and preserveAspectRatio of the SVG element
  ...
  if(bohrungen >= 1) createCircle(svg, svgns, platteAbstand, platteAbstand, bohrungenDurchmesser); // Top-left corner
  if(bohrungen >= 2) createCircle(svg, svgns, lange - platteAbstand, platteAbstand, bohrungenDurchmesser); // Top-right corner
  if(bohrungen >= 3) createCircle(svg, svgns, lange - platteAbstand, breite - platteAbstand, bohrungenDurchmesser); // Bottom-right corner
  if(bohrungen >= 4) createCircle(svg, svgns, platteAbstand, breite - platteAbstand, bohrungenDurchmesser); // Bottom-left corner
...
function createCircle(svg, svgns, cx, cy, diameter) {
