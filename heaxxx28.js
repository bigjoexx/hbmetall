...
function calculateVorgrundierungRotPrice(pricesData) {
  const checkbox = document.getElementById("vorgrundierung-rot");
  if (!checkbox.checked) {
    return 0;
  }
...
function calculateTotalPrice(selectedValues, pricesData, traegerMenge, kopfplattePrice, fusplattePrice, finalWeight) {
  if (selectedValues.laenge <= 0 || selectedValues.traegerMenge <= 0) {
    return { totalPrice: 0, feuerverzinkungPrice: 0, vorgrundierungRotPrice: 0 };
  }
  ...
  const feuerverzinkungPrice = calculateFeuerverzinkungPrice(selectedValues, pricesData, finalWeight);
    console.log("FeuerverzinkungPrice:", feuerverzinkungPrice);
  const vorgrundierungRotPrice = calculateVorgrundierungRotPrice(pricesData);
  ...
  const totalPrice = (laengePrice + optionsPrice + baseCuttingPrice + kopfplattePrice + fusplattePrice + feuerverzinkungPrice + vorgrundierungRotPrice) * traegerMenge;
    console.log("Total price:", totalPrice);
  
  return { totalPrice, feuerverzinkungPrice, vorgrundierungRotPrice };
}
