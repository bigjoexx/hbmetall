...
if (bohrungen >= 1) createCircle(svg, svgns, platteAbstand + radius, platteAbstand + radius, bohrungenDurchmesser); // Top-left corner
if (bohrungen >= 2) createCircle(svg, svgns, lange - platteAbstand - radius, breite - platteAbstand - radius, bohrungenDurchmesser); // Bottom-right corner
if (bohrungen >= 3) createCircle(svg, svgns, lange - platteAbstand - radius, platteAbstand + radius, bohrungenDurchmesser); // Top-right corner
if (bohrungen >= 4) createCircle(svg, svgns, platteAbstand + radius, breite - platteAbstand - radius, bohrungenDurchmesser); // Bottom-left corner
...
const computeMaxPlatteAbstand = () => {
  const lange = parseInt(document.getElementById("kopfplatte-lange").value, 10) || 0;
  const breite = parseInt(document.getElementById("kopfplatte-breite").value, 10) || 0;
  const bohrungenDurchmesser = parseInt(document.getElementById("kopfplatte-bohrungen-durchmesser").value, 10);
  const radius = bohrungenDurchmesser / 2;

  // The maximum platteAbstand should ensure that the holes don't overlap,
  // and it should be such that the circles are always inside the rectangle.
  // This is achieved when the distance is the smaller one between half of the smaller dimension of the plate,
  // and the distance between the center of the plate and the radius of the hole.
  return Math.min(Math.min(lange, breite) / 2, (Math.min(lange, breite) - 2 * radius) / 2);
};


// Function to handle input change
const handleInputChange = () => {
  let platteAbstand = (parseInt(langeInput.value, 10) + parseInt(breiteInput.value, 10)) / 2;

  // Compute maximum allowable platteAbstand
  const maxPlatteAbstand = computeMaxPlatteAbstand();

  // Make sure platteAbstand does not exceed the maximum value
  if (platteAbstand > maxPlatteAbstand) {
    platteAbstand = maxPlatteAbstand;
  }
