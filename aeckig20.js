...
// Variables to track previous and current thickness
let previousThickness = thickness;
let currentThickness = thickness;

// Get the current selected value
const selectedValueKehlnahtstarke = kehlnahtstarkeSelection.value;

// Remove all options
kehlnahtstarkeSelection.innerHTML = '';

// Function to update the options based on thickness
function updateOptions() {
  // Clear the options
  kehlnahtstarkeSelection.innerHTML = '';

  if (currentThickness < 6) {
    // Thickness is below 6, so add '3 mm' and '5 mm'
    kehlnahtstarkeSelection.add(new Option('3 mm', '3 mm'));
    kehlnahtstarkeSelection.add(new Option('5 mm', '5 mm'));
  } else {
    // Thickness is 6 or above, so add '5 mm' and '7 mm'
    kehlnahtstarkeSelection.add(new Option('5 mm', '5 mm'));
    kehlnahtstarkeSelection.add(new Option('7 mm', '7 mm'));
  }

  // Set the pre-selected value based on the previous thickness
  if (previousThickness < 6) {
...
// Add an event listener to detect when the thickness changes
kehlnahtstarkeSelection.addEventListener('change', () => {
  previousThickness = currentThickness;
  currentThickness = parseFloat(thickness); // Assuming thickness is a numeric value

  // Check if the thickness has crossed the threshold of 6
  if ((previousThickness < 6 && currentThickness >= 6) || (previousThickness >= 6 && currentThickness < 6)) {
    // If it crossed the threshold, update the options
    updateOptions();
  }
});

// Add an event listener to temporarily remove the change listener when the user interacts
kehlnahtstarkeSelection.addEventListener('mousedown', () => {
  kehlnahtstarkeSelection.removeEventListener('change', () => {
    previousThickness = currentThickness;
...
// Add an event listener to restore the change listener when the user releases the mouse button
kehlnahtstarkeSelection.addEventListener('mouseup', () => {
  kehlnahtstarkeSelection.addEventListener('change', () => {
    previousThickness = currentThickness;
    currentThickness = parseFloat(thickness);
...
function updateOptions() {
  ...
  // Set the pre-selected value based on the selectedValueKehlnahtstarke
