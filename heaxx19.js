...
function calculateLengthPrice(format, beamLength, pricesData) {
  if (!pricesData.hasOwnProperty(format)) {
    console.error("HEA size not found in prices data:", format);
    return 0;
  }

  const kgPerMeter = parseFloat(pricesData[format]["kg/m"]);
  const beamLengthInMeters = beamLength / 1000;
  const weight = kgPerMeter * beamLengthInMeters;
  const pricePerKg = parseFloat(pricesData[format]["kg"]);
...
function calculateBeamTotalWeight(selectedValues, pricesData) {
  const format = selectedValues.format;
...
function calculateOptionsPrice(selectedValues, pricesData) {
  ...
  const stegblechePrice = parseFloat(pricesData[format]["stegbleche"]) * heaStegbleche;
  const ausklinkungenPrice = parseFloat(pricesData[format]["ausklinkungen"]) * heaAusklinkungen;
  const bugelPrice = parseFloat(pricesData[format]["bugel"]) * heaBugel;
  ...
  const weiteresPrice = parseFloat(pricesData[format][weiteresOption]);
...
const bohrungenPricePerHole = parseFloat(pricesData[format][diameterCategory]);
...
function calculateTotalPrice(selectedValues, pricesData, beamMenge, kopfplattePrice, fusplattePrice) {
  ...
  const lengthPrice = calculateLengthPrice(selectedValues.format, parseInt(selectedValues.beamLength), pricesData);
  ...
  const baseCuttingPrice = parseFloat(pricesData[selectedValues.format]["schneiden"]);
...
async function main() {
  ...
  document.getElementById("format").addEventListener("change", () => updatePrice(pricesData));
