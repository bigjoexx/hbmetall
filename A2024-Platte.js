...
const kopfplatteBohrungenSelect = document.getElementById('kopfplatte-bohrungen');
  ...
  function getProductData() {
    ...
    const imageUrl = "https://uploads-ssl.webflow.com/6401aa505499a5c8687df64f/64fae16eecc183a3dddd06e4_platte-final--watermark.png"; // Replace with the CMS image URL for the product
...
async function fetchKopfplattePrices() {
  // Use your Personal Access Token and Airtable URL
  const personalAccessToken = 'patBxEFMXqAnjBTHi.90152106ab2c7a037160f1c83ad6e263b34ac12dc4aa0a622c4f64c0b71812a8';
  const url = 'https://api.airtable.com/v0/appIIKEo5ExPVPr9I/Stahlplatte';
  ...
  records.forEach((record) => {
    ...
    prices[record.fields["Option"]][value] = record.fields["Price"];
  });
...
function calculateKopfplattePrice(kopfplattePricesData, steelDensity) {
  ...
  const buegel = parseInt(document.getElementById("kopfplatte-buegel").value, 10);
  ...
  const buegelPrice = parseFloat(kopfplattePricesData["buegel"][buegel.toString()]);

  const kopfplattePrice = sizePrice + bohrungenPrice + baseCuttingPriceKopfplatte + dornePrice + buegelPrice;
...
function calculateFinalWeight(kopfplattePricesData, traegerMenge, steelDensity) {
  ...
  const finalWeight = kopfplatteWeight.toFixed(2);
...
function calculateFinalSurface(traegerMenge, kopfplattePricesData) {
  ...
  const finalSurface = kopfplatteSurface.toFixed(2);
...
function calculateFeuerverzinkungPrice(kopfplattePricesData, steelDensity) {
  ...
  const startPrice = parseFloat(kopfplattePricesData["Feuerverzinkung start"]["default"]);
  ...
  const feuerverzinkungPrice = finalWeight * pricePerKg + startPrice;
...
function calculateVorgrundierungRotPrice(kopfplattePricesData, steelDensity) {
  ...
  const vorgrundierungRotPrice = vorgrundierungRotPricePerSquareMeter;
...
function calculateVorgrundierungGrauPrice(kopfplattePricesData, steelDensity) {
  ...
  const vorgrundierungGrauPrice = vorgrundierungGrauPricePerSquareMeter;
...
function calculateTotalPrice(kopfplattePricesData, traegerMenge, laenge, steelDensity, finalWeight, finalSurface) {
  ...
  const totalPrice = Math.round(kopfplattePrice + feuerverzinkungPrice + vorgrundierungRotPrice + vorgrundierungGrauPrice);
...
function updateMinDistance() {
  ...
  if ([1, 2, 3, 4].includes(parseInt(numHoles))) {
    ...
    if (parseInt(distanceField.value, 10) < minDistance || distanceField.value === '') {
...
abstandInput.addEventListener("blur", function() {
  setTimeout(() => {
    const min = parseInt(this.getAttribute("min"), 10);
    if (parseInt(this.value, 10) < min || this.value === '') {
      this.value = min;
      updatePrice(steelDensity, kopfplattePricesData);  // adjust SVG as per updated value.
    }
  }, 0);
});
...
function createSVG(platteAbstand) {
  ...
  const bohrungen = parseInt(document.getElementById("kopfplatte-bohrungen").value, 10);
  const bohrungenDurchmesser = parseInt(document.getElementById("kopfplatte-bohrungen-durchmesser").value, 10);
  const bohrungenPosition = document.getElementById("kopfplatte-bohrungen-position").value;
  ...
  // Create holes (circles) at corners
const radius = bohrungenDurchmesser / 2;
if (bohrungen === 1) {
        createCircle(svg, svgns, platteAbstand + radius, platteAbstand + radius, bohrungenDurchmesser); // Top-left corner
    } else if (bohrungen === 2) {
        if (bohrungenPosition === "mittig") {
            createCircle(svg, svgns, lange / 2, platteAbstand + radius, bohrungenDurchmesser); // Middle top
            createCircle(svg, svgns, lange / 2, breite - platteAbstand - radius, bohrungenDurchmesser); // Middle bottom
        } else if (bohrungenPosition === "diagonale") {
            createCircle(svg, svgns, platteAbstand + radius, platteAbstand + radius, bohrungenDurchmesser); // Top-left corner
            createCircle(svg, svgns, lange - platteAbstand - radius, breite - platteAbstand - radius, bohrungenDurchmesser); // Bottom-right corner
        }
    } else if (bohrungen === 3) {
        createCircle(svg, svgns, platteAbstand + radius, platteAbstand + radius, bohrungenDurchmesser); // Top-left corner
        createCircle(svg, svgns, lange - platteAbstand - radius, breite - platteAbstand - radius, bohrungenDurchmesser); // Bottom-right corner
        createCircle(svg, svgns, lange - platteAbstand - radius, platteAbstand + radius, bohrungenDurchmesser); // Top-right corner
    } else if (bohrungen === 4) {
        createCircle(svg, svgns, platteAbstand + radius, platteAbstand + radius, bohrungenDurchmesser); // Top-left corner
        createCircle(svg, svgns, lange - platteAbstand - radius, breite - platteAbstand - radius, bohrungenDurchmesser); // Bottom-right corner
        createCircle(svg, svgns, lange - platteAbstand - radius, platteAbstand + radius, bohrungenDurchmesser); // Top-right corner
        createCircle(svg, svgns, platteAbstand + radius, breite - platteAbstand - radius, bohrungenDurchmesser); // Bottom-left corner
    }
...
const handleInputChange = () => {
  ...
  window.requestAnimationFrame(() => createSVG(platteAbstand));
};
...
async function updatePrice(steelDensity, checkboxChanged = false) {
  ...
  updateDisplayedPrices(oldPriceWithoutVAT, oldPriceWithVAT, totalPrice, traegerMenge);
  window.requestAnimationFrame(() => createSVG(platteAbstand));
}
...
async function main() {
  ...
  document.getElementById("kopfplatte-bohrungen-position").addEventListener("input", () => updatePrice(steelDensity, kopfplattePricesData));
  document.getElementById("traeger-menge").addEventListener("change", () => updatePrice(steelDensity, kopfplattePricesData));
  ...
  document.getElementById("platte-abstand").addEventListener("change", () => updatePrice(steelDensity, kopfplattePricesData));
  document.getElementById("kopfplatte-buegel").addEventListener("change", () => updatePrice(steelDensity, kopfplattePricesData));
  document.getElementById("kopfplatte-dorne").addEventListener("change", () => updatePrice(steelDensity, kopfplattePricesData));
