  ...
  function getProductData() {
    ...
    const productCode = "hbrund"; // Replace with the CMS code for the product
    const productName = "Hohlprofile Rund"; // Replace with the CMS name for the product
    const imageUrl = "https://uploads-ssl.webflow.com/6401aa505499a5c8687df64f/64fae16716cb7a65adefd365_rund-watermark.png"; // Replace with the CMS image URL for the product
...
async function fetchPrices() {
  ...
  const url = 'https://api.airtable.com/v0/appIIKEo5ExPVPr9I/RUND';
...
function calculateLaengePrice(traegerFormat, laenge, pricesData) {
  ...
  const pricePerMeter = pricePerKg * kgPerMeter;
...
async function fetchKopfplattePrices() {
  ...
  const personalAccessToken = 'patBxEFMXqAnjBTHi.90152106ab2c7a037160f1c83ad6e263b34ac12dc4aa0a622c4f64c0b71812a8';
  const url = 'https://api.airtable.com/v0/appIIKEo5ExPVPr9I/Kopfplatte';
...
let selectedValueKehlnahtstarke = kehlnahtstarkeSelection.value;

// Get the available options based on thickness
const availableOptions = (thickness < 6) ? ['3 mm', '5 mm'] : ['5 mm', '7 mm'];

// Remove all options
kehlnahtstarkeSelection.innerHTML = '';

// Add the available options to the select element
availableOptions.forEach(optionValue => {
  kehlnahtstarkeSelection.add(new Option(optionValue, optionValue));
});

// Set the selected value to the previously selected value if it's still available
if (availableOptions.includes(selectedValueKehlnahtstarke)) {
  kehlnahtstarkeSelection.value = selectedValueKehlnahtstarke;
}
...
function calculatePrice() {
  ...
  // You can perform other actions here based on the selected option and price
}
...
kehlnahtstarkeSelection.addEventListener('change', () => {
  selectedValueKehlnahtstarke = kehlnahtstarkeSelection.value;
  // Calculate the price when the option changes
  calculatePrice();
});
...
function calculateKopfplatteSurface(selectedValues) {
  ...
  // Use your Personal Access Token and Airtable URL
  const personalAccessToken = 'patBxEFMXqAnjBTHi.90152106ab2c7a037160f1c83ad6e263b34ac12dc4aa0a622c4f64c0b71812a8';
  const url = 'https://api.airtable.com/v0/appIIKEo5ExPVPr9I/Fusplatte';
...
function calculateTotalPrice(selectedValues, pricesData, traegerMenge, kopfplattePrice, fusplattePrice, finalWeight, finalSurface) {
  ...
  const totalPrice = Math.round((laengePrice + baseCuttingPrice + kopfplattePrice + fusplattePrice) + feuerverzinkungPrice + vorgrundierungRotPrice + vorgrundierungGrauPrice);
