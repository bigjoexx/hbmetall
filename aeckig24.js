...
// Define the available options based on thickness
const availableOptions = [];
if (thickness < 6) {
  availableOptions.push({ text: '3 mm', value: '3 mm' });
  availableOptions.push({ text: '5 mm', value: '5 mm' });
} else {
  availableOptions.push({ text: '5 mm', value: '5 mm' });
  availableOptions.push({ text: '7 mm', value: '7 mm' });
}

// Add the available options to the select element
availableOptions.forEach(option => {
  kehlnahtstarkeSelection.add(new Option(option.text, option.value));
});

// Set the pre-selected value based on the available options
if (availableOptions.some(option => option.value === selectedValueKehlnahtstarke)) {
  kehlnahtstarkeSelection.value = selectedValueKehlnahtstarke;
} else {
  // If the previously selected value is not available, select the first option
  kehlnahtstarkeSelection.value = availableOptions[0].value;
}
